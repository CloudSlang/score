...
public class WorkerRecoveryServiceImpl implements WorkerRecoveryService, LoginListener {
    ...
    private static final int DEFAULT_POLL_SIZE = getInteger("worker.recoveryMaxPollSize", 100);

    static final private long maxAllowedGap = getLong("max.allowed.version.gap.msg.recovery", 10); //This is the max allowed gap
    ...
    @Override
    @Transactional
    public void doWorkerRecovery(String workerUuid, boolean shouldPurgeQueues) {

        // this is needed because if there are execution messages left for finished executions in the queues
        // they will be recovered over and over again that causes the worker to end up in endless recovery
        purgeExecutionQueuesForFinishedExecutions(shouldPurgeQueues);
    ...
    private int getMessagesWithoutAck(int maxSize, String workerUuid) {
        if (logger.isDebugEnabled()) {
            logger.debug("Getting messages count without ack for worker: " + workerUuid);
        }
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Messages without ack found: " + result + " for worker: " + workerUuid);
        }

        return result;
    }

    private void purgeExecutionQueuesForFinishedExecutions(boolean shouldPurgeQueues) {
        if (shouldPurgeQueues) {
            try {
                executionQueueService.purgeExecutionQueuesForFinishedExecutions();
            } catch (Exception e) {
                logger.warn("Failed to purge execution queues for finished executions", e);
            }
        }
    }
