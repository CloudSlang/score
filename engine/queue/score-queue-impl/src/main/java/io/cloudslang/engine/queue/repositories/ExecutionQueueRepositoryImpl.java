...
/*
    private void deleteFromQueues(List ids) {

        String query = QUERY_DELETE_FINISHED_STEPS_FROM_QUEUES.replaceAll(":ids", StringUtils.repeat("?", ",", ids.size()));
    ...
    private void deleteStateAndMappings(List ids) {

        String query = QUERY_DELETE_EXECS_STATES_MAPPINGS.replace(":ids", StringUtils.repeat("?", ",", ids.size()));
        Object[] args = ids.toArray(new Object[ids.size()]);
        logSQL(query, args);
