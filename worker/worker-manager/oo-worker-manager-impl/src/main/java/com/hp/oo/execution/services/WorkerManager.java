 ...
 * @version $Id$
 */
public class WorkerManager implements ApplicationListener<ApplicationEvent>, EndExecutionCallback {
	...
	private void init() {
		...
		executorService = new ThreadPoolExecutor(numberOfThreads, numberOfThreads, Long.MAX_VALUE, TimeUnit.NANOSECONDS, inBuffer, new WorkerThreadFactory("WorkerExecutionThread"));
	...
	private void doStartup() {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				workerNodeService.updateEnvironmentParams(workerUuid, System.getProperty("os.name"), System.getProperty("java.version"), resolveDotNetVersion());
			}
		}).start();
	}
