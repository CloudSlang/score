...
public class SimpleExecutionRunnable implements Runnable {
	...
	public void run() {
		...
		if (executionMessage != null){
			if (logger.isDebugEnabled()) logger.debug("go to InBuffer after " + counter + " steps");
			inBuffer.addExecutionMessage(executionMessage);
		} else if (logger.isDebugEnabled()) logger.debug("end of thread after " + counter + " steps");
	}
		...
		try {
			...
			if (executionService.isSplitStep(execution)) {
				...
				if (logger.isDebugEnabled()) {
					...
					if (nextStepExecution != null) logger.debug("Next position: " + nextStepExecution.getPosition());
				}
