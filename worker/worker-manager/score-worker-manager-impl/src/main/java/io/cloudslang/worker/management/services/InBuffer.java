...
/** Created with IntelliJ IDEA. User: kravtsov Date: 20/11/12 Time: 08:46 */
public class InBuffer implements WorkerRecoveryListener, ApplicationListener, Runnable {
  ...
  private static final int MINIMUM_GC_DELTA =
      10000; // minimum delta between garbage collections in milliseconds
  ...
  private double startPollingMemoryRatio =
      0.2D; // ratio out of max memory with which workers can start polling for messages
    ...
    startPollingMemoryRatio =
        Double.valueOf(
            System.getProperty(
                ...
                String.valueOf(startPollingMemoryRatio)));
    logger.info(
    ...
    while (!inShutdown) {
      try {
        ...
        if (!workerUp) {
          ...
          syncManager
              .startGetMessages(); // we must lock recovery lock before poll - otherwise we will get
          // duplications

          // We need to check if the current thread was interrupted while waiting for the lock
          // (InBufferThread) and RESET its interrupted flag!
          ...
          long workerFreeMemory = workerManager.getFreeMemory();
          if (needToPoll(workerFreeMemory)) {
            int maxMessagesToGet =
                capacity
                    - workerManager
                        .getInBufferSize(); // TODO - think of removing it now that we limit by
            // memory

            if (logger.isDebugEnabled())
              logger.debug("Polling messages from queue (max " + maxMessagesToGet + ")");
            List<ExecutionMessage> newMessages =
                queueDispatcher.poll(workerUuid, maxMessagesToGet, memoryForPolling);
            if (!newMessages.isEmpty()) { // todo delete this after testing
              logger.info(newMessages.size() + " messages got polled for worker " + workerUuid);
              newMessages.forEach(
                  (msg) ->
                      logger.info(
                          "Message id "
                              ...
                              + " and payload_size "
                              + msg.getPayloadSize()));
              logger.info("Worker molling is " + memoryForPolling);
            }
            if (executionsActivityListener != null) {
              executionsActivityListener.onActivate(
                  extract(newMessages, on(ExecutionMessage.class).getExecStateId()));
            }
            if (logger.isDebugEnabled())
            ...
            Thread.sleep(
                coolDownPollingMillis); // if the buffer is not empty enough yet or in recovery -
            // sleep a while
          }
        }
  ...
  private boolean needToPoll(long workerFreeMemory) {
    int bufferSize = workerManager.getInBufferSize();
    if (logger.isDebugEnabled()) {
      logger.debug("InBuffer size: " + bufferSize);
      logger.debug("Worker free memory: " + workerFreeMemory + " bytes");
    }
    return bufferCanTakeMoreExecutions(bufferSize)
        && isWorkerMemoryOverPollingThreshold(startPollingMemoryRatio, workerFreeMemory);
  }
  ...
  private void ackMessages(List<ExecutionMessage> newMessages) throws InterruptedException {
    ...
    for (ExecutionMessage message : newMessages) {
      // create a unique id for this lane in this specific worker to be used in out buffer
      // optimization
      ...
      cloned =
          (ExecutionMessage)
              message
                  .cloneWithoutPayload(); // payload is not needed in ack - make it null in order to
                                          // minimize the data that is being sent
      cloned.setStatus(ExecStatus.IN_PROGRESS);
      cloned.incMsgSeqId();
      message
          .incMsgSeqId(); // increment the original message seq too in order to preserve the order
                          // of all messages of entire step
  ...
  public void addExecutionMessage(ExecutionMessage msg) throws InterruptedException {
    try {
      syncManager
          .startGetMessages(); // this is a public method that can push new executions from outside
                               // - from execution threads
  ...
  public boolean isWorkerMemoryOverPollingThreshold(
      final double startPollingMemoryRatio, final long freeMemory) {
  ...
  public void doRecovery() {
    // We must interrupt the inBuffer thread in case it is stuck in await() because the outBuffer is
    // full
