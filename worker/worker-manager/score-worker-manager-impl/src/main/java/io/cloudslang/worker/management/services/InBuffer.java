...
public class InBuffer implements WorkerRecoveryListener, ApplicationListener, Runnable {
        ...
        while (!inShutdown) {
            try {
                ...
                if (!workerUp) {
                    ...
                    syncManager
                            .startGetMessages(); //we must lock recovery lock before poll - otherwise we will get duplications
                    ...
                    if (needToPoll()) {
                        ...
                        if (!newMessages.isEmpty()) {
                            ...
                            for (ExecutionMessage msg : newMessages) {
                                addExecutionMessageForPolling(msg);
                            }
                        ...
                        Thread.sleep(
                                coolDownPollingMillis); //if the buffer is not empty enough yet or in recovery - sleep a while
                    }
                }
    ...
    private void ackMessages(List<ExecutionMessage> newMessages) throws InterruptedException {
        ...
        for (ExecutionMessage message : newMessages) {
            ...
            cloned.setPayload(
                    null); //payload is not needed in ack - make it null in order to minimize the data that is being sent
    ...
    public void addExecutionMessage(ExecutionMessage msg) throws InterruptedException {
        try {
            syncManager
                    .startGetMessages(); //this is a public method that can push new executions from outside - from execution threads
    ...
    private void addExecutionMessageForPolling(ExecutionMessage msg) throws InterruptedException {
        SimpleExecutionRunnable simpleExecutionRunnable = simpleExecutionRunnableFactory.getObject();
        simpleExecutionRunnable.setExecutionMessage(msg);
        long executionId = parseLong(msg.getMsgId());

        workerManager.addExecutionForPolling(simpleExecutionRunnable, executionId,
